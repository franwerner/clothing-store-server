...
import UserAdresessModel from "@/model/users/userAddresess.model";
...
class UserAddresessService {
    ...
    static async createAddress(address: UserAddressesSchema.Insert): Promise<UserAddressesSchema.Base> {
        ...
        const [insertID] = await UserAdresessModel.insert(parseData)
        return {
            ...parseData,
    ...
    static async getAddress(user_fk: DatabaseKeySchema) {
        const [address] = await UserAdresessModel.select({ user_fk })
        if (!address) {
